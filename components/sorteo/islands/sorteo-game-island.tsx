...
export function SorteoGameIsland({
  ...
  // Fallback title logic for SSR if props missing (same as MainApp)
  if (!hasHydrated && !initialTitle && seoMode) {
      if (seoMode === 'yes-no') { displayTitle = tYesNo('h1'); displaySubtitle = tYesNo('subtitle') }
      ...
      else if (seoMode === 'wheel') { displayTitle = tWheel('h1'); displaySubtitle = tWheel('subtitle') }
  }
