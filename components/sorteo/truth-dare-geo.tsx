...
import { ArrowRight, Flame, Shuffle, Smartphone, HelpCircle, CheckCircle, Trophy } from "lucide-react"
...
export function TruthGeo() {
  ...
  const tPage = useTranslations("TruthPage")
  ...
  const faqs = [
    {
      question: tPage("faq_1_q"),
      answer: tPage("faq_1_a"),
    },
    {
      question: tPage("faq_2_q"),
      answer: tPage("faq_2_a"),
    },
  ]
            ...
            <div className="prose prose-invert max-w-none">
              <p className="text-lg text-muted-foreground leading-relaxed">
                {t.rich("direct_answer_text", {
                  ...
                  tool: (chunks) => <Link href="/truth-or-dare-generator" className="font-semibold text-primary hover:underline">{chunks}</Link>,
              ...
              <ol className="relative border-l border-primary/20 ml-3 space-y-8">
                 {howToSteps.map((step, idx) => (
                     <li key={idx} className="ml-8">
                        ...
                        <p className="text-lg text-foreground/90 font-medium">{step}</p>
                     </li>
                 ))}
              </ol>
           </div>
