...
import { Shield, Zap, HelpCircle, Lock } from "lucide-react"
import { JsonLd } from '@/components/seo/json-ld'

export function PasswordGeo() {
  const t = useTranslations("PasswordGeo")
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="prose prose-invert prose-lg max-w-none text-muted-foreground leading-relaxed p-6 bg-background/40 rounded-2xl border border-border/50">
                {t.rich('direct_answer_text', {
                    tool: (chunks) => <strong className="text-foreground font-semibold bg-primary/10 px-1 rounded">{chunks}</strong>,
                    brand: (chunks) => <strong className="text-primary font-bold">{chunks}</strong>,
            ...
            <ol className="space-y-4" role="list">
                {[1, 2, 3, 4].map((step) => (
                    <li key={step} className="flex gap-4 items-start p-5 bg-card/60 rounded-xl border border-border/50 list-none">
                        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-green-500 to-emerald-500 text-white flex items-center justify-center font-bold text-sm shadow-lg shadow-green-900/20">
                            {step}
                        </div>
