...
export async function GET(request: Request) {
  try {
    ...
    // Viralis: Parse Shared List (Deep Linking)
    let participants: string[] = [];
    if (listParam) {
        try {
            // It might be double encoded or just encoded once, MainApp uses encodeURIComponent
            // But searchParams.get does one decode automatically usually?
            // Let's try direct parse first, if fails, decode.
            // MainApp: encodeURIComponent(JSON.stringify(names))
            const parsed = JSON.parse(listParam);
            if (Array.isArray(parsed)) {
                participants = parsed.map(String).slice(0, 50); // Hard limit for safety
            }
        } catch (e) {
            try {
                 // Fallback if it needs manual decoding
                 const parsed = JSON.parse(decodeURIComponent(listParam));
                 if (Array.isArray(parsed)) {
                    participants = parsed.map(String).slice(0, 50);
                 }
            } catch (e2) {
                console.error("Failed to parse list param for OG", e2);
            }
        }
    }
    ...
    // MODE 3: LIST PREVIEW (Participants list present, no winner yet)
            ...
            (
                ...
                >
                     ...
                     <div style={{ zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', width: '90%' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 40 }}>
                             ...
                             <div style={{
                                ...
                                {theme.icon}
                                <span style={{ fontSize: 24, fontWeight: 700, color: theme.accentColor }}>
                        ...
                        <div style={{
                            ...
                            borderRadius: 24,
                            border: `1px solid ${theme.subTextColor}33`,
                            padding: '30px',
                            width: '800px',
                            ...
                            gap: '16px'
                             ...
                             {participants.slice(0, 5).map((p, i) => (
                                 <div key={i} style={{
                                     ...
                                     fontWeight: 500,
                                     color: theme.subTextColor,
                                     borderBottom: i < 4 && i < participants.length - 1 ? `1px solid ${theme.subTextColor}22` : 'none',
                                     paddingBottom: i < 4 && i < participants.length - 1 ? 12 : 0
                                 }}>
                                     <div style={{
                                         display: 'flex',
                                         width: 8,
                     ...
                     <div style={{
                         ...
                         <span style={{ fontSize: 20, fontWeight: 700, letterSpacing: '2px', color: 'white' }}>SORTEO PRO</span>
                     </div>
                </div>
            ),
            { width: 1200, height: 630 }
