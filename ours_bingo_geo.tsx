...
import { ArrowRight, Lock, Zap, RefreshCw, Smartphone, HelpCircle, CheckCircle, Grid, Volume2, ShieldCheck } from "lucide-react"
...
export function BingoGeo() {
  ...
  const faqs = [
    {
      question: bingoFaqT("faq_1_q"),
      answer: bingoFaqT("faq_1_a"),
    },
    {
      question: bingoFaqT("faq_2_q"),
      answer: bingoFaqT("faq_2_a"),
    },
  ]

  const howToSteps = [
    { name: bingoT('how_to_step_1') },
    { name: bingoT('how_to_step_2') },
    { name: bingoT('how_to_step_3') },
    { name: bingoT('how_to_step_4') },
  ]
        ...
        >
          ...
          <div className="mb-10 space-y-4">
            <h2 className="text-3xl md:text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-amber-200">
              {bingoT("direct_answer_title")}
            </h2>
            <div className="prose prose-invert max-w-none">
              <p className="text-lg text-muted-foreground leading-relaxed">
                {bingoT.rich("direct_answer_text", {
                  strong: (chunks) => <strong className="font-semibold text-foreground">{chunks}</strong>,
                  tool: (chunks) => <Link href="/bingo-number-generator" className="font-semibold text-primary hover:underline">{chunks}</Link>,
            ...
            <Button asChild size="lg" className="mt-4 gap-2 text-lg font-bold shadow-lg shadow-primary/20">
              <a href="#sorteo-section" onClick={handleTryIt}>
                {bingoT("cta_button")} <ArrowRight className="w-5 h-5" />
              </a>
            </Button>
          </div>
          ...
          <ul className="grid md:grid-cols-2 gap-8 mt-12 mb-16" role="list">
            <li className="space-y-4">
              <h3 className="text-xl font-bold flex items-center gap-2 text-primary">
                ...
                {bingoT("feature_1_title")}
              </h3>
              <p className="text-muted-foreground">{bingoT("feature_1_desc")}</p>
            </li>

            <li className="space-y-4">
              <h3 className="text-xl font-bold flex items-center gap-2 text-primary">
                <Grid className="w-5 h-5" />
                {bingoT("feature_2_title")}
              </h3>
              <p className="text-muted-foreground">{bingoT("feature_2_desc")}</p>
            </li>

            <li className="space-y-4">
              <h3 className="text-xl font-bold flex items-center gap-2 text-primary">
                <RefreshCw className="w-5 h-5" />
                {bingoT("feature_3_title")}
              </h3>
              <p className="text-muted-foreground">{bingoT("feature_3_desc")}</p>
            </li>

            <li className="space-y-4">
              <h3 className="text-xl font-bold flex items-center gap-2 text-primary">
                <Volume2 className="w-5 h-5" />
                {bingoT("feature_4_title")}
              </h3>
              <p className="text-muted-foreground">{bingoT("feature_4_desc")}</p>
            </li>
          </ul>
