...
export default async function TruthOrDareGeneratorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'TruthPage' });
  const tGeo = await getTranslations({ locale, namespace: 'TruthGeo' });
  const tShare = await getTranslations({ locale, namespace: 'ShareContent' });
  const tWinner = await getTranslations({ locale, namespace: 'WinnerCeremony' });
  const tGame = await getTranslations({ locale, namespace: 'TruthOrDare' });
  ...
  // Fetch truths and dares dynamically
  // We assume 20 keys each (0-19) as per spec
  const truths = Array.from({ length: 20 }, (_, i) => tGame(`truth_${i}`));
  const dares = Array.from({ length: 20 }, (_, i) => tGame(`dare_${i}`));
  ...
  return (
    <>
      <script
        ...
        dangerouslySetInnerHTML={{ __html: safeJsonLdStringify([softwareAppSchema, breadcrumbSchema, faqSchema]) }}
      />
      <MainApp
