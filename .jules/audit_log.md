...
| [Mon Apr  6 09:33:10 UTC 2026] | FCP / LCP | Removed framer-motion from static seo-content.tsx | Reduced client bundle size and prevented layout shifts on static text components. |2026-04-20 | Performance Optimization | Split MainApp in Truth or Dare Generator | Deferred MainApp loading to improve FCP for the truth-or-dare-generator route.
| [2024-05-25] | FCP < 1.0s | Refactored AppSkeleton to map initialStyle to specific visualization skeletons instead of hardcoded SlotSkeleton, and passed visualization prop dynamically from page.tsx files | E2E CWV tests passing with strict FCP threshold |
[2026-06-08] | Main Thread Sovereignty | Fixed synchronous setState in useEffect across multiple components (verify-content, sorteo-cards, cascade, sticky-footer, winner-ceremony, use-mobile) by deferring them with requestAnimationFrame and implementing proper cleanups via cancelAnimationFrame. | Eliminated 8 ESLint warnings, reduced cascading renders during hydration.
[2026-06-29] | Next.js Architecture | Added generateStaticParams to dynamic route [locale]/verify/page.tsx | Resolved SSG bailout and restored static rendering performance.
| 2026-07-13 | Main Thread Sovereignty | Fixed synchronous setState in useEffect in `share-button.tsx` and `main-app.tsx` by deferring them with requestAnimationFrame and implementing proper cleanups via cancelAnimationFrame. | Eliminated ESLint warnings, reduced cascading renders during hydration.
