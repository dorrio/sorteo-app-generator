...
export default async function TruthOrDareGeneratorPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  // Fetch questions
  const tTruths = await getTranslations({ locale, namespace: 'Truths' });
  const tDares = await getTranslations({ locale, namespace: 'Dares' });

  // Generate arrays assuming keys 0-19 exist
  const truths = Array.from({ length: 20 }, (_, i) => tTruths(`${i}`));
  const dares = Array.from({ length: 20 }, (_, i) => tDares(`${i}`));
