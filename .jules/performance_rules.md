...
*   **Safe JSON-LD:** Use `safeJsonLdStringify` from `@/lib/utils` instead of `JSON.stringify` to prevent XSS and ensure proper formatting.
*   **Clean Component Composition:** Client Component wrappers (like `MainApp`) should accept heavy features as `children` and not import them statically or use fallback logic.
...
*   **Unsafe JSON-LD:** Using `JSON.stringify` in `dangerouslySetInnerHTML`.
